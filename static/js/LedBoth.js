...
function bothLuminosityChanged(event) {
    ...
    if (canSendData()) {
        ...
        luminosityTimeout = setTimeout(function () { bothLuminosityChanged(event); }, 10);
    }
};

// Lors du clique sur le bouton pour éteindre la bande
function bothClickOnOff(event) {
...
function bothRgbValueChanged(event) {
...
function bothPickedColor(event){
    ...
    if (canSendData()) {
        ...
        colorTimeout = setTimeout(function () { bothPickedColor(event); }, 10);
    }
};