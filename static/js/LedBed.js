...
function bedLuminosityChanged(event) {
    ...
    if (canSendData()) {
        ...
        luminosityTimeout = setTimeout(function () { bedLuminosityChanged(event); }, 10);
    }
};

// Lors du clique sur le bouton pour éteindre la bande
function bedClickOnOff(event) {
...
function bedRgbValueChanged(event) {
...
function bedPickedColor(event){
    ...
    if (canSendData()) {
        ...
        colorTimeout = setTimeout(function () { bedPickedColor(event); }, 10);
    }
};